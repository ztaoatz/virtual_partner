...
def enhanced_chat(request):
    ...
    try:
        ...
        print(f"用户的消息数: {ChatMessage.objects.filter(user=user).count()}")
...
def get_chat_history(request):
    ...
    print(f"请求参数: user_id={user_id}, session_id={session_id}")
    ...
    try:
        ...
        try:
            profile = UserProfile.objects.get(external_user_id=user_id)
            user = profile.user
            print(f"找到用户: {user.username}, Profile ID: {profile.id}")
        except UserProfile.DoesNotExist:
