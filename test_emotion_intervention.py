...
def create_test_emotional_scenarios():
    ...
    user, created = User.objects.get_or_create(
        ...
        defaults={
            'email': 'intervention_test@example.com',
            'first_name': '情绪干预',
            'last_name': '测试用户'
        }
    )
    ...
    scenarios = {
        "mild": {
            "name": "轻度干预场景",
            "scores": [50, 44, 43, 42],  # 连续3天低于45分，当前42分
...
def test_diary_generation_with_intervention():
    ...
    for i, score in enumerate(low_scores):
        test_date = base_date - timedelta(days=len(low_scores)-i)
        EmotionDiary.objects.create(
            user=user,
            date=test_date,
            content=f"历史日记 - 分值{score}",
            emotions=json.dumps([{"emotion": "沮丧", "intensity": score}]),
            main_topic="测试",
            message_count=3,
            emotion_score=score
        )
    ...
    # 现在生成今天的日记，应该会触发干预
    api_url = "http://127.0.0.1:8000/generate-diary/"
    payload = {
        'user_id': str(profile.external_user_id),
